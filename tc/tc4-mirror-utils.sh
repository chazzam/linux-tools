...
TC_TCZ_64="4.x/x86_64/tcz";
TC_RELEASE_64="4.x/x86_64/release";
TC_5_TCZ="5.x/x86/tcz";
TC_5_RELEASE="5.x/x86/release";

# We need to know the kernel version for dependency resolution.
TC_KERNEL_VERSION="3.0.21-tinycore";
TC_KERNEL_VERSION_64="3.0.21-tinycore64";
TC_5_KERNEL_VERSION="3.8.10-tinycore";
TC_5_KERNEL_VERSION_64="3.8.10-tinycore64";
...
search_dependencies() {
  ...
  MISSING="$({ ls -- *.tcz; ls -- *.tcz; cat *.dep|sed -e 's/.tcz/.tcz\n/'|sort|uniq|sed s/KERNEL/$TC_KERNEL_VERSION/|tr -d ' '; } | sed -e 's/asterisk*.tcz//;s/dahdi*.tcz//;s/libpri*.tcz//;s/libnewt*.tcz//;' | sed -e s/.tcz// | sort | uniq -u )";
}
...
update_available_package_list() {
  ...
  for i in `ls -1 -- *.info|sort -f`; do
    tmp="$(basename -- "$i" .info)";
    desc="$(grep Description $i | sed -e s/Description:\\s\\+//)";
    echo "$tmp" >> info.lst
...
[ -d "${TC_TCZ_64}" ] || mkdir -p "${TC_TCZ_64}";
[ -d "${TC_5_TCZ}" ] || mkdir -p "${TC_5_TCZ}";
cd ${TC_TCZ};
...
if [ "download" == "$op" ]; then
  ...
  cd ${TC_5_TCZ};
  TC_TCZ=$TC_5_TCZ
  TC_KERNEL_VERSION=$TC_5_KERNEL_VERSION
  download_and_verify_packages $args;
elif [ "update" == "$op" ]; then
  # Update packages
...
if [ "update" == $op ]; then
  ...
  # mount and extract core.gz and vmlinuz and put them in the release directory

  cd ${MIRROR_DIR};
  [ -d "${TC_5_RELEASE}" ] || mkdir -p "${TC_5_RELEASE}";
  cd ${TC_5_RELEASE};
  $WGET -O Core-current.iso -- ${TC_MIRROR}/${TC_5_RELEASE}/Core-current.iso;
fi;

# Handle 64bit
cd ${MIRROR_DIR}/${TC_TCZ_64};
TC_KERNEL_VERSION="$TC_KERNEL_VERSION_64";
TC_TCZ="$TC_TCZ_64";
...
if [ "update" == $op ]; then
  echo "Downloading updated 64bit boot";
  # If we're going to update, then update the image we're building with as well.
  cd ${MIRROR_DIR};
  [ -d "${TC_RELEASE_64}" ] || mkdir -p "${TC_RELEASE_64}";
  cd ${TC_RELEASE_64};
  TC_RELEASE_64="4.x/x86_64"; # their setup isn't the same yet...
  $WGET -O corepure64.gz -- ${TC_MIRROR}/${TC_RELEASE_64}/corepure64.gz;
  $WGET -O corepure64.gz.md5.txt -- ${TC_MIRROR}/${TC_RELEASE_64}/corepure64.gz.md5.txt;
  $WGET -O vmlinuz64 -- ${TC_MIRROR}/${TC_RELEASE_64}/vmlinuz64;
  $WGET -O vmlinuz64.md5.txt -- ${TC_MIRROR}/${TC_RELEASE_64}/vmlinuz64.md5.txt;
fi;
